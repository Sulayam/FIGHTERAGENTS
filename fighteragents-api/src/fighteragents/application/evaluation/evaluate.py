...
from fighteragents.application.conversation_service.generate_response import get_response
from fighteragents.application.conversation_service.workflow import state_to_str
...
async def evaluation_task(x: dict) -> dict:
    """Calls agentic app logic to evaluate ufcfighter responses.
    ...
    Args:
        x: Dictionary containing evaluation data with the following keys:
            ...
            ufcfighter_id: ID of the ufcfighter to use
    ...
    Returns:
        dict: Dictionary with evaluation results containing:
            ...
            output: Generated response from ufcfighter
            expected_output: Expected answer for comparison
    """

    ufcfighter_factory = UFCFighterFactory()
    ufcfighter = ufcfighter_factory.get_ufcfighter(x["ufcfighter_id"])
...
def get_used_prompts() -> list[opik.Prompt]:
    ...
    prompts = [
        client.get_prompt(name="ufcfighter_character_card"),
...
def evaluate_agent(
    ...
    Args:
        dataset: Dataset containing evaluation examples.
            Must contain messages and ufcfighter_id.
